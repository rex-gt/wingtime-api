...
# Register as admin so all endpoints are accessible
...
check "POST /api/users/register" "201" "$STATUS"
...
$CURL -X DELETE "$BASE_URL/api/members/$MEMBER_ID" -H "Authorization: Bearer $TOKEN" > /dev/null
...
STATUS=$($CURL -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/users/register" \
  -H "Content-Type: application/json" \
  -d "{\"first_name\":\"Smoke\",\"last_name\":\"Test\",\"email\":\"$EMAIL\",\"password\":\"$PASSWORD\"}")
check "POST /api/users/register" "201" "$STATUS"
...
[ -n "$TOKEN" ] && [ "$TOKEN" != "null" ] && check "POST /api/users/login (token received)" "ok" "ok" || check "POST /api/users/login (token received)" "ok" "fail"
...
[ -n "$AIRCRAFT_ID" ] && [ "$AIRCRAFT_ID" != "null" ] && check "POST /api/aircraft (id returned)" "ok" "ok" || check "POST /api/aircraft (id returned)" "ok" "fail"
...
[ -n "$RESERVATION_ID" ] && [ "$RESERVATION_ID" != "null" ] && check "POST /api/reservations (id returned)" "ok" "ok" || check "POST /api/reservations (id returned)" "ok" "fail"

STATUS=$($CURL -o /dev/null -w "%{http_code}" "$BASE_URL/api/reservations/$RESERVATION_ID" \
  -H "Authorization: Bearer $TOKEN")
check "GET /api/reservations/:id" "200" "$STATUS"

# Conflict — same aircraft, same time
STATUS=$($CURL -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/api/reservations" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"member_id\":$MEMBER_ID,\"aircraft_id\":$AIRCRAFT_ID,\"start_time\":\"2026-06-01T09:00:00Z\",\"end_time\":\"2026-06-01T11:00:00Z\"}")
...
[ -n "$FLIGHTLOG_ID" ] && [ "$FLIGHTLOG_ID" != "null" ] && check "POST /api/flight-logs (id returned)" "ok" "ok" || check "POST /api/flight-logs (id returned)" "ok" "fail"
...
[ -n "$BILLING_ID" ] && [ "$BILLING_ID" != "null" ] && check "POST /api/billing/generate (id returned)" "ok" "ok" || check "POST /api/billing/generate (id returned)" "ok" "fail"
